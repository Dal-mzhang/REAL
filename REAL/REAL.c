 ...
 *  March    2021       M. Zhang    Release version 1.3
 ************************************************************************/
...
int main(int argc, char **argv){
    ...
    latcenter = 0.0; //avoid using fixed multiplication of 111.19 km/deg, change with your latcenter (suggested by Ruijia Wang) 
	...
	//Usage
    if(argc < 3 || error == 1){
        fprintf(stderr, "Usage:  Rapid Earthquake Association and Location (REAL1.3, March 2021)\n");
    ...
    if(latref0 > -999 && lonref0 > -999){
        fprintf(stderr,"searching range: %lf %lf %lf %lf\n",latref0-rx1,latref0+rx1,lonref0-rx2,lonref0+rx2);
        fprintf(stderr,"                 lat_range=+-%lf lon_range=+-%lf\n",rx1,rx2);
        fprintf(stderr,"                 lat_grid=%lf lon_grid=%lf\n",dx1,dx2);
    }else{
...
//homo model
if(igrid == 0){
    #pragma omp parallel for shared(pscounts) firstprivate(latref,lonref,latref0,lonref0,elevref,nlon,ndep,dx1,dx2,dh) private(lat0,lon0,dep,l,i,j,k)
    for(l=0; l<nnn; ++l){
        i = (int)(l/(nlon*ndep));
